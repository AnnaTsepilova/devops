...
# Створюємо фіксований IP для NAT gateway
  ...
  tags = merge(
    {
      "Name" = format(
        "${local.name}-nat-ip-%s",
        format("%s%s", data.aws_region.current.region, element(var.availability_zones, count.index)),
      )
    },
    local.tags
  )
...
# Створюємо роути для NAT Gateway
